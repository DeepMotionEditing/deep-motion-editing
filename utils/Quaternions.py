...
class Quaternions:
    ...
    def __init__(self, qs):
        if isinstance(qs, np.ndarray):
            ...
            # 至少是一个二维的数组——即使只有一个quaternion，也让它成一个长度为1的数组
    ...
    def __mul__(self, other):
        ...
        3. This is scaling a Quaternion
        ...
        if isinstance(other, np.ndarray) and other.shape[-1] == 3:
            # 这时候是q * (0, x, y, z) * q^{-1}, 再取imaginaries
    ...
    # Directly negating all parts of Quaternion q results in q' that has the same function
    # To preserve some uniqueness here, they used the one with positive real part
    def __abs__(self):
    ...
    # discard the last dim (for 4 parts of one Quaternion)
    @property
